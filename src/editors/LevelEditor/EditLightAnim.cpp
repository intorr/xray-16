...
//---------------------------------------------------------------------------
__fastcall TfrmEditLightAnim::TfrmEditLightAnim(TComponent* Owner) : TForm(Owner)
{
...
{
    m_Props =
        TProperties::CreateForm("LAProps", paProps, alClient, TOnModifiedEvent(this, &TfrmEditLightAnim::OnModified));
    m_Items = TItemList::CreateForm(
        "LA Items", paItems, alClient, TItemList::ilEditMenu | TItemList::ilDragAllowed | TItemList::ilFolderStore);
...
//---------------------------------------------------------------------------
void __fastcall TfrmEditLightAnim::FormCloseQuery(TObject* Sender, bool& CanClose)
{
    if (!bFinalClose)
        CanClose = IsClose();
    else
    ...
    if (Shift.Contains(ssLeft))
        if (iMoveKey >= 0)
        {
            ...
            if ((new_key != iTgtMoveKey) && (new_key < m_CurrentItem->iFrameCount) &&
                (!m_CurrentItem->IsKey(new_key) || (new_key == iMoveKey)))
            {
                iTgtMoveKey = new_key;
                pbG->Repaint();
            }
